...
// From http://stackoverflow.com/questions/2690328/qt-quncompress-gzip-data
bool GzipCompress(const QByteArray& input, QByteArray& output, int level) {
  ...
  if (!input.isEmpty()) {
    ...
    // Initialize deflater
    int ret = deflateInit2(&strm, qMax(-1, qMin(9, level)), Z_DEFLATED,
                           GZIP_WINDOWS_BIT, 8, Z_DEFAULT_STRATEGY);
    ...
    do {
      ...
      const int chunk_size = qMin(GZIP_CHUNK_SIZE, input_data_left);
      ...
      do {
        ...
        const int have = (GZIP_CHUNK_SIZE - strm.avail_out);
        // Cumulate result
        if (have > 0) {
